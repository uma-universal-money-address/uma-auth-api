// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

/*
 * UMA Auth API
 *
 * This API allows you to authenticate with the UMA server to take actions on a user's wallet. It's the exposed communication layer between the NWC server and the main UMA server.
 *
 * API version: 0.1
 */

package umaauth


import (
	"errors"
)



// TransactionFx - Foreign exchange rate for the transaction.
type TransactionFx struct {

	ReceivingCurrency Currency `json:"receiving_currency,omitempty"`

	SendingCurrency Currency `json:"sending_currency,omitempty"`

	// The total amount that will be received in the smallest unit of the receiving currency (eg. cents).
	TotalReceivingAmount int64 `json:"total_receiving_amount,omitempty"`

	// The total amount that will be sent in the smallest unit of the sending currency (eg. cents).
	TotalSendingAmount int64 `json:"total_sending_amount,omitempty"`

	// Number of sending currency units per receiving currency unit.
	Multiplier float32 `json:"multiplier,omitempty"`

	// The fees associated with the quote in the smallest unit of the sending currency (eg. cents).
	Fees int64 `json:"fees,omitempty"`
}

// AssertTransactionFxRequired checks if the required fields are not zero-ed
func AssertTransactionFxRequired(obj TransactionFx) error {
	if err := AssertCurrencyRequired(obj.ReceivingCurrency); err != nil {
		return err
	}
	if err := AssertCurrencyRequired(obj.SendingCurrency); err != nil {
		return err
	}
	return nil
}

// AssertTransactionFxConstraints checks if the values respects the defined constraints
func AssertTransactionFxConstraints(obj TransactionFx) error {
	if err := AssertCurrencyConstraints(obj.ReceivingCurrency); err != nil {
		return err
	}
	if err := AssertCurrencyConstraints(obj.SendingCurrency); err != nil {
		return err
	}
	if obj.TotalReceivingAmount < 0 {
		return &ParsingError{Param: "TotalReceivingAmount", Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.TotalSendingAmount < 0 {
		return &ParsingError{Param: "TotalSendingAmount", Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.Multiplier < 0 {
		return &ParsingError{Param: "Multiplier", Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.Fees < 0 {
		return &ParsingError{Param: "Fees", Err: errors.New(errMsgMinValueConstraint)}
	}
	return nil
}
